...
**Canal de communication — abstraction provider [DÉCISION — 26 juillet 2026].** Toute communication client sortante passe par un **canal de communication** rattaché au prestataire, jamais par un numéro WhatsApp codé en dur dans les services métier. Un canal porte un `provider_kind` (`whatsapp_sidian` aujourd'hui ; `whatsapp_business_personal` prévu pour un WhatsApp Business personnel du prestataire) et un identifiant opaque. **Les services métier sélectionnent un canal (défaut ou explicite) ; ils ne connaissent ni ne manipulent le numéro d'expédition.** Un seul provider est implémenté au départ : **WhatsApp Sidian** (numéro / compte plateforme). L'ajout d'un WhatsApp Business personnel devra s'effectuer comme un nouvel adaptateur derrière la même interface — sans refonte du modèle créance / agent / relance. Le détail technique vit en 03.

**Plans et canaux WhatsApp [DÉCISION — cadrage commercial, 26 juillet 2026] :**
- **Starter / Early Access :** canal **WhatsApp Sidian** partagé (plateforme) — seul canal livré dans G1-P.
- **Pro (non implémenté dans G1-P) :** connexion d'un WhatsApp Business personnel du Guide.
- **Business (non implémenté) :** plusieurs canaux / numéros / collaborateurs.

**Confirmation Guide via WhatsApp [DÉCISION — G1-Q, 26 juillet 2026].** Le Guide peut répondre au message `guide_payment_confirmation` (Oui / Non / Paiement partiel / Je vérifie). Ces réponses produisent des commandes métier traçables sur un **état de confirmation Guide** dédié. Elles **ne créent pas** encore un règlement `paiement` / ne mutent pas le solde créance via `detecte_hors_sidian` (toujours hors MVP auto-détection). Une déclaration manuelle auditée (`declare_manuellement_hors_sidian`) reste une migration produit à part. « Je vérifie » n’annule pas un prélèvement programmé et ne suspend pas les échéances (comportement prudent — suspension éventuelle à trancher produit).
